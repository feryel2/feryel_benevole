...
{//bool test=false;
    QSqlQuery query;
          ...
          query.prepare("INSERT INTO benevole (id_benevole,nom,prenom,tel,adresse) "
 ...
 bool Benevole::supprimer(int id)
 {
     ...
     query.prepare("Delete From Benevole where id_benevole = :id");
 ...
 {
      QSqlQuery query;
     query.prepare("update BENEVOLE set id_benevole=:id_benevole,nom=:nom,prenom=:prenom,tel=:tel ,adresse=:adresse where id_benevole=:id_benevole");
     query.bindValue(":id_benevole",id_benevole);
     query.bindValue(":nom",nom);
     query.bindValue(":prenom",prenom);
     query.bindValue(":tel",tel);
 ...
 QSqlQueryModel* Benevole:: tester()
 {
    ...
    model->setQuery("SELECT id_benevole FROM Benevole");// ce modele contient les id Benevole des Benevole seulement
    //model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID Benevole"));
    return model;
 }
 ...
 QSqlQueryModel *Benevole::rechercher(QString rech)
 {
     QSqlQueryModel * model= new QSqlQueryModel();
         ...
         model->setQuery("select * from Benevole where id_benevole LIKE '"+rech+"%'");
     return model;
 }
